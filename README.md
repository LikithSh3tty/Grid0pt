...
Three details of that were settled by measurement rather than argument, all using the certificate below as the arbiter. The weight is `g(f) = f²`, which reaches the proven optimum on 67 of the 72 corpus instances against the linear weight's 65 and is worse on none. For rectangular cells the quarter turn is solved unconditionally, because a rectilinear outline gives the vote no oblique chords to reason from at all. R comes out 0 and the grid would stay put, even though swapping a 2×3 cell to 3×2 is a different tiling and often the better one. And the local refine **proves rather than probes**: it runs the same branch and bound described below over a narrow window around the voted angle, because the probes it replaced were 1.25° apart while the answer can sit 0.03° from the vote. It costs nothing where it cannot help, since the bound is checked first and short-circuits it, so the median request is unchanged.

Together those took the pipeline from missing the proven optimum on 7 of the corpus's 72 instances to missing on **none of them**.
...
The vote changes job rather than being replaced: it supplies the starting placement, and the search supplies the proof that nothing beats it. Usually the vote was already right and the search only confirms it: 15 windows to prove that 108 cells is the most a 3×3 grid can ever fit in a 36×27 room tilted 23°. A curved boundary is the expensive case, since the count barely varies with angle and nothing prunes on quality. Proving the whole placement period costs tens of seconds against under one, so that is opt-in; a search that runs out of budget reports the gap it could not close rather than claiming optimality. The refine above uses the same machinery on a much narrower window, which is why it can be on by default.
...
Run the Python ones from `backend/`, since the modules import each other flatly, so the repository root is the wrong working directory.

The frontend tests cover what the components decide (which request goes out, which optimality verdict is shown) rather than how they paint. They were written after the components rather than before, so each was checked by breaking the code it covers and confirming it failed; a test that has never failed has not been shown to test anything.
...
- **The paper deliberately cites no prior work, for now.** It renders from `python -m evaluation.paper` and serves as a reference document while the work continues; the real write-up comes once the project is finished. Its final section records the gap so that whoever writes that version starts from an accurate account of what has and has not been attributed. Not an open task.
