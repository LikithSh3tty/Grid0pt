...
- **The exact refine stays opt-in, and cost is not why.** `refine="certified"` reaches placements the sampled refine cannot resolve — it closes the last corpus instance, where the optimum sits 0.03° from the voted angle while the probes are 1.25° apart. But it finds more cells on **none** of the sixteen quick-corpus instances, and exactly one in 72 across the full one. Cost is now close — 0.70s mean against the probing refine's 0.30s, because the bound is checked first and proves the refine pointless on most shapes before any work is done — so what stops it being the default is the value, not the price.
