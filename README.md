...
- **The exact refine stays opt-in, and cost is not why.** `refine="certified"` reaches placements the sampled refine cannot resolve — it closes the last corpus instance, where the optimum sits 0.03° from the voted angle while the probes are 1.25° apart. But measured across the quick corpus it finds more cells on **none** of the sixteen instances while costing four times the mean and up to a minute, and across the full corpus it gains exactly one instance in 72. Making it cheaper would not change that; what would is a way to tell in advance which instances are the knife-edge ones.
