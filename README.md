...
- **Find why the vote misses on rectangular cells.** Certifying the full corpus put the remaining 5 misses all at 2×3 and none at square cells. With a rectangular cell a wall flush with the width and the same wall flush with the height are different placements, and the vote has to pick one; the certificate already names the placement it should have found, so the diagnosis is localised and only the fix is missing.
